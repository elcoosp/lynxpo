...
package com.{{org}}.{{project_name|camel_case}}.modules.network
...
class NetworkModule(private val context: Context) : LynxpoModule(context) {
  ...
  override fun definition() = ModuleDefinition {
    ...
    @LynxMethod
    fun getNetworkStateAsync(callback: Callback) {
      return callback.invoke(LynxUIMethodConstants.SUCCESS, fetchNetworkState())
    }
    

    @LynxMethod
    fun getIpAddressAsync(callback: Callback): String {
      return callback.invoke(LynxUIMethodConstants.SUCCESS, rawIpToString(wifiInfo.ipAddress))
    }
    

    @LynxMethod
    fun isAirplaneModeEnabledAsync(callback: Callback): Boolean {
      return callback.invoke(LynxUIMethodConstants.SUCCESS, Settings.Global.getInt(context.contentResolver, Settings.Global.AIRPLANE_MODE_ON, 0) != 0)
    }
