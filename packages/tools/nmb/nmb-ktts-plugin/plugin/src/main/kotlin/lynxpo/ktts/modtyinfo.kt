...
import modtyinfo.ClassInfo
import modtyinfo.ConstructorInfo
import modtyinfo.EnumValue
import modtyinfo.MethodInfo
import modtyinfo.PackageInfo
import modtyinfo.ParameterInfo
import modtyinfo.PropertyDefinition
import modtyinfo.SerializableTypeInfo
import modtyinfo.SourceLocation
import modtyinfo.TypeAliasInfo
import modtyinfo.TypeCategory
import modtyinfo.TypeInfo
import modtyinfo.TypeInfoFile
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        allFiles.forEach { file ->
            ...
            val moduleAnnotations =
                mutableMapOf<String, String>()    // Extract imports from file content
...
//            }
            ...
            val collector = SymbolCollector(classes, typeAliases, resolver)
    ...
    private inner class SymbolCollector(
        ...
        override fun visitClassDeclaration(classDeclaration: KSClassDeclaration, data: Unit) {
            ...
            val classInfo = processClassDeclaration(classDeclaration, resolver)
        ...
        override fun visitTypeAlias(typeAlias: KSTypeAlias, data: Unit) {
            ...
            typeAliases.add(processTypeAlias(typeAlias, resolver))
        }
    }
    ...
    private fun processClassDeclaration(
        ...
        val implementedTypes = classDeclaration.superTypes
            ...
            .map { createTypeInfo(it, resolver) }
            .toList()
        ...
        val superclass = classDeclaration.superTypes
            ...
            ?.let { createTypeInfo(it, resolver) }
        ...
        classDeclaration.declarations
            ...
            .forEach { function ->
                if (function.isConstructor()) {
                    constructors.add(processConstructor(function, resolver))
                } else {
                    methods.add(processFunction(function, resolver))
                }
            }
        ...
        classDeclaration.declarations
            ...
            .forEach { property ->
                properties.add(processProperty(property, resolver))
            }
        // TODO: add to schema
        ...
        classDeclaration.declarations
            ...
            .forEach { nestedClass ->
                ...
                val nestedTypeInfo =
                    createTypeInfo(
                        resolver.createKSTypeReferenceFromKSType(nestedClass.asStarProjectedType()),
                        resolver
                    )
                ...
                val nestedCollector = SymbolCollector(nestedClasses, nestedTypeAliases, resolver)
        ...
        val typeParameters =
            classDeclaration.typeParameters.map { processTypeParameter(it, resolver) }.toList()

        // Handle serializable types if necessary
        if (classDeclaration.isSerializable()) {
            serializableTypes.add(processSerializableType(classDeclaration, resolver))
        }
    ...
    private fun processTypeAlias(typeAlias: KSTypeAlias, resolver: Resolver): TypeAliasInfo {
        val typeParameters =
            typeAlias.typeParameters.map { processTypeParameter(it, resolver) }.toList()

        return TypeAliasInfo(
            name = typeAlias.simpleName.asString(),
            full_name = typeAlias.qualifiedName?.asString() ?: "",
            underlying_type = createTypeInfo(typeAlias.type, resolver),
    ...
    private fun processFunction(function: KSFunctionDeclaration, resolver: Resolver): MethodInfo {
        val parameters = function.parameters.map { processParameter(it, resolver) }.toList()
        val typeParameters =
            function.typeParameters.map { processTypeParameter(it, resolver) }.toList()
        ...
        return MethodInfo(
            ...
            return_type = createTypeInfo(function.returnType, resolver),
            parameters = parameters,
            receiver_type = function.extensionReceiver?.let { createTypeInfo(it, resolver) },
    ...
    private fun processConstructor(
        ...
        val parameters = constructor.parameters.map { processParameter(it, resolver) }.toList()
        val annotations = mutableMapOf<String, String>()
    ...
    private fun processParameter(parameter: KSValueParameter, resolver: Resolver): ParameterInfo {
        val annotations = mutableMapOf<String, String>()
        ...
        return ParameterInfo(
            ...
            type = createTypeInfo(parameter.type, resolver),
    ...
    private fun processProperty(
        ...
        return PropertyDefinition(
            ...
            type = createTypeInfo(property.type, resolver),
    ...
    private fun processSerializableType(
        classDeclaration: KSClassDeclaration,
        resolver: Resolver
    ): SerializableTypeInfo {
        val properties = classDeclaration.declarations
            .filterIsInstance<KSPropertyDeclaration>()
            .map { processProperty(it, resolver) }
            .toList()
        ...
        val constructors = classDeclaration.declarations
            ...
            .map { processConstructor(it, resolver) }
            .toList()

        val typeParameters = classDeclaration.typeParameters
            .map { processTypeParameter(it, resolver) }
            .toList()

        val nestedTypes = classDeclaration.declarations
            .filterIsInstance<KSClassDeclaration>()
            .map {
                val tyRef = resolver.createKSTypeReferenceFromKSType(it.asStarProjectedType())
                createTypeInfo(tyRef, resolver)
            }
            .toList()
    ...
    private fun createTypeInfo(typeRef: KSTypeReference?, resolver: Resolver): TypeInfo {
        if (typeRef == null) {
            return TypeInfo(
        ...
        val typeArguments = resolvedType.arguments.map { arg ->
            arg.type?.let { createTypeInfo(it, resolver) } ?: TypeInfo(
        ...
        return TypeInfo(
            ...
            type_parameters = if (declaration is KSClassDeclaration) {
                declaration.typeParameters.map { processTypeParameter(it, resolver) }.toList()
            } else {
                emptyList()
            }
        )
