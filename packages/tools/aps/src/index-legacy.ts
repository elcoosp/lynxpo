...
abstract class PermissionsScraper<T extends PermissionInfo> {
  ...
  constructor(config: Partial<ScraperConfig>, defaultConfig: ScraperConfig) {
    ...
    if (fs.existsSync(this.config.outputFile)) {
      try {
        this.permissions = JSON.parse(fs.readFileSync(this.config.outputFile, 'utf-8'));
        console.log(`Loaded ${this.permissions.length} existing permissions from ${this.config.outputFile}`);
      } catch (error) {
        console.error(`Error loading existing permissions from ${this.config.outputFile}:`, error);
        this.permissions = [];
      }
    }
  }
  ...
  protected getCategoryFromUrl(url: string): string[] {
    ...
    Object.entries(this.config.categoryKeywords).forEach(([keyword, category]) => {
  ...
  public saveToFile(filename?: string): void {
    const outputPath = path.resolve(process.cwd(), filename || this.config.outputFile);
  ...
  protected abstract processPermissionPage(page: Page, url: string): Promise<T | null>;
  
  // The main scraping method - common implementation but calls abstract methods
  public async scrapePermissions(): Promise<T[]> {
    try {
      await this.initBrowser();
      console.log(`Fetching main page from ${this.config.baseUrl}${this.config.mainPagePath}...`);
      ...
      const newPermissionLinks = this.filterNewPermissionLinks(allPermissionLinks);
      
      console.log(`Found ${allPermissionLinks.length} permission links (${newPermissionLinks.length} new to process).`);
      
      // Process permissions in parallel batches
      const batches = [];
      for (let i = 0; i < newPermissionLinks.length; i += this.config.maxConcurrentRequests) {
        batches.push(newPermissionLinks.slice(i, i + this.config.maxConcurrentRequests));
      }
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        console.log(`Batch ${i+1} completed. Processed ${batchResults.length} permissions in this batch.`);
      ...
      console.log(`Successfully processed ${newPermissions.length} new permissions. Total: ${this.permissions.length}`);
...
class ApplePermissionsScraper extends PermissionsScraper<PermissionInfo> {
  ...
  constructor(config: Partial<AppleScraperConfig> = {}) {
    ...
    if (config.patterns) {
      this.appleConfig.patterns = { ...ApplePermissionsScraper.DEFAULT_CONFIG.patterns, ...config.patterns };
    }
    if (config.defaults) {
      this.appleConfig.defaults = { ...ApplePermissionsScraper.DEFAULT_CONFIG.defaults, ...config.defaults };
    }
  }
  ...
  protected extractPermissionLinks(markdown: string): string[] {
    ...
    while ((match = linkRegex.exec(markdown)) !== null) {
      ...
      if (linkText.toLowerCase().includes('UsageDescription'.toLowerCase()) || 
  ...
  private extractInfoFromMarkdown(markdown: string, url: string): {
    ...
    const keyNameMatch = markdown.match(this.appleConfig.patterns.keyNameBullet);
    if (keyNameMatch) {
      result.keyName = keyNameMatch[1].trim();
    } else {
      // Try to find it from a header
      const headerMatch = markdown.match(this.appleConfig.patterns.keyNameHeader);
      if (headerMatch) {
    ...
    const descriptionMatch = markdown.match(this.appleConfig.patterns.description);
    if (descriptionMatch) {
      result.description = descriptionMatch[1].trim();
    }
    
    // Check for deprecation notice
    const deprecationMatch = markdown.match(this.appleConfig.patterns.deprecationNotice);
    if (deprecationMatch) {
      result.deprecationNote = deprecationMatch[1].trim();
    }
    
    // Extract iOS version information
    const iosVersionMatch = markdown.match(this.appleConfig.patterns.iosVersion);
    if (iosVersionMatch) {
    ...
    const discussionMatch = markdown.match(this.appleConfig.patterns.discussion);
    if (discussionMatch) {
      ...
      if (discussionText.includes('required') || 
    ...
    const codeMatches = markdown.match(this.appleConfig.patterns.deviceCapabilities);
    if (codeMatches) {
      codeMatches.forEach(match => {
        const capability = match.replace(/`/g, '').trim();
        if (capability.match(/^[a-z-]+$/) || 
            capability.includes('UIRequiredDeviceCapabilities')) {
          const cleanCapability = capability.replace('UIRequiredDeviceCapabilities', '').trim();
          if (cleanCapability && !result.deviceCapabilities.includes(cleanCapability)) {
  ...
  protected async processPermissionPage(page: Page, url: string): Promise<PermissionInfo | null> {
    try {
      ...
      if (!info.keyName || !info.keyName.includes('UsageDescription')) {
        console.log(`Skipping ${url} - not a usage description key or could not extract key name`);
        return null;
      }
      
      // Check if we already have this permission
      if (this.permissionExists(info.keyName)) {
        console.log(`Skipping ${url} - permission ${info.keyName} already exists in dataset`);
        return null;
      }
      
      // Check for deprecation
      const isDeprecated = info.deprecationNote.length > 0 || markdown.includes('Deprecated');
      ...
      if (!description) {
        ...
        const descriptionAltMatch = markdown.match(this.appleConfig.patterns.alternateDescription);
      ...
      const permissionInfo: PermissionInfo = {
        ...
        platforms: {
          ios: {
            ...
            minIosVersion: info.minIosVersion || this.appleConfig.defaults.minIosVersion,
          }
        }
      };
  ...
  protected async processPermissionPage(page: Page, url: string): Promise<PermissionInfo | null> {
    ...
    try {
      ...
      if (this.permissionExists(permissionName)) {
        console.log(`Skipping ${url} - permission ${permissionName} already exists in dataset`);
        return null;
      }
      
      // Extract other details (implement based on actual page structure)
      const descriptionMatch = html.match(this.androidConfig.patterns.description);
      const description = descriptionMatch ? descriptionMatch[1] : '';
      
      const protectionLevelMatch = html.match(this.androidConfig.patterns.protectionLevel);
      ...
      const minSdk = minSdkMatch ? parseInt(minSdkMatch[1]) : this.androidConfig.defaults.minSdk;
      
      const constantValueMatch = html.match(this.androidConfig.patterns.constantValue);
      const constantValue = constantValueMatch ? constantValueMatch[1] : '';
      
      // Check for deprecation
      const deprecationMatch = html.match(this.androidConfig.patterns.deprecationNotice);
      const isDeprecated = !!deprecationMatch || html.includes('This constant was deprecated');
      ...
      const permissionInfo: PermissionInfo = {
        id: permissionId,
        name: permissionName.replace(/^PERMISSION_/, '').replace(/_/g, ' ').toLowerCase(),
        description: description,
        deprecated: isDeprecated,
        replacement: null,
        categories: categories,
      ...
      if (isDeprecated && deprecationMatch) {
        ...
        permissionInfo.deprecationInfo = {
          deprecatedSince: apiMatch ? parseInt(apiMatch[1]) : undefined,
          platforms: {
            android: { 
              sinceApi: apiMatch ? parseInt(apiMatch[1]) : 0, 
              removalApi: 0 
            }
          }
        };
...
async function main() {
  ...
  const appleScraper = new ApplePermissionsScraper({...customConfig,
    outputFile: 'apple-permissions.json',
  });
  const androidScraper = new AndroidPermissionsScraper({...customConfig,
    outputFile: 'android-permissions.json',
  });
