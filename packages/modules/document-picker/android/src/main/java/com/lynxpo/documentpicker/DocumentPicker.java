// Copyright 2026 The Lynxpo Authors. All rights reserved.
// Licensed under the Apache License Version 2.0 that can be found in the
// LICENSE file in the root directory of this source tree.
package com.lynx.explorer.modules;

import android.content.Context;
import com.lynx.jsbridge.LynxMethod;
import com.lynx.jsbridge.LynxModule;
import com.lynx.jsbridge.LynxNativeModule;
import com.lynxpo.documentpicker.generated.DocumentPickerSpec;
import com.lynx.react.bridge.WritableMap;
import com.lynx.react.bridge.JavaOnlyMap;

/**
 * Android counterpart of the iOS DocumentPicker. Exposes functionality to JS via
 * NativeModules.DocumentPicker, faithfully porting Expo's native method surface.
 */
@LynxNativeModule(name = "DocumentPicker")
public class DocumentPicker extends DocumentPickerSpec {

  public DocumentPicker(Context context) {
    super(context);
  }

  @LynxMethod
  public WritableMap documentAsync(String options) {
    WritableMap m = new JavaOnlyMap();
    m.putString("source", "LynxPo: DocumentPicker.documentAsync (stub)");
    m.putBoolean("available", true);
    return m;
  }

  @LynxMethod
  public boolean isAvailableAsync() {
    return true;
  }

}
